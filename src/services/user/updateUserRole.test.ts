...
describe('updateUserRole', () => {
  ...
  it('should throw a BadRequestError if userId is missing', async () => {
    ...
    await expect(updateUserRole(mockUserUpdate)).rejects.toThrow(MESSAGES.MISSING_USER_ID)
    expect(db.update).not.toHaveBeenCalled()
  })

  it('should throw a BadRequestError if role is missing', async () => {
    mockUserUpdate.role = null as unknown as ROLES
    await expect(updateUserRole(mockUserUpdate)).rejects.toThrow(MESSAGES.ROLE_REQUIRED)
    expect(db.update).not.toHaveBeenCalled()
  })
